...
`Note` 지그비나 블루투스 연결시 RC-100 은 녹색LED 가 켜지며, RC-100A, RC-100B는 파란색 LED 가 켜집니다.{: .notice}

`Note` AX-S1과의 적외선 통신은 지원되지 않습니다. {: .notice}

`Note` ZIG110 지그비는 지원되지 않습니다.{: .notice}

`Note` 무선모듈을 2개를 동시에 연결하지 마세요. {: .notice}
...
1. RC-100 의 건전지 뚜껑을 열면 아래와 같이 ZIG-100 이라고 적힌 곳이 ZIG-100 모듈을 장착할 위치입니다.
...
- [ RoboPlus Task](/docs/kr/edu/ollo/bugkit/#zigbee-wireless-control) 를 통해 로봇을 조종하는 프로그램을 만들 수 있습니다.


### Zigbee SDK

- [Zigbee SDK](/docs/kr/software/sdk/zigbee_sdk/)를 통해 C언어로 ZIGBee를 제어할 수 있습니다.
